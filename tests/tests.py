...
class TestViews(TestCase):
    ...
    def setUp(self):
        ...
        post = Post.objects.create(title="Hello", text="World", category=category)
        post.tag.add(tag)
    ...
    def test_index_get(self):
        ...
        self.assertQuerysetEqual(response.context["post_list"], ["<Post: Hello>"])
    ...
    def test_category_get_2(self):
        ...
        try:
            self.client.get(reverse("blog:category", kwargs={"category": "aiueo"}))
    ...
    def test_tag_get_1(self):
        """/tag/Django アクセスのテスト."""
        response = self.client.get(reverse("blog:tag", kwargs={"tag": "Django"}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Hello")
        self.assertQuerysetEqual(response.context["post_list"], ["<Post: Hello>"])
    ...
    def test_comment_get_1(self):
        """/comment/1 アクセスのテスト."""
        response = self.client.get(reverse("blog:comment", kwargs={"pk": 1}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_comment_get_2(self):
        ...
        """
        response = self.client.get(reverse("blog:comment", kwargs={"pk": 2}))
        self.assertEqual(response.status_code, 404)
